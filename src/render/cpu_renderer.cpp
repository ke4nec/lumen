...
// Placeholder 5x7 bitmap font for ASCII 32..126 (plan §4.3: 占位文本).
// Each glyph is 7 rows; bit 4 of a row is the leftmost pixel. Non-ASCII code
// points render as boxes so CJK content still occupies the measured width.
constexpr std::uint8_t kFont5x7[95][7] = {
    ...
    {0b01110, 0b10001, 0b00001, 0b01101, 0b10101, 0b10101, 0b01111},  // @
    ...
    {0b10000, 0b10010, 0b10100, 0b11000, 0b10100, 0b10010, 0b10001},  // k
    ...
    {0b00000, 0b00000, 0b01111, 0b10000, 0b10000, 0b10000, 0b10000},  // r
    {0b00000, 0b00000, 0b01111, 0b10000, 0b01110, 0b00001, 0b11110},  // s
    {0b01000, 0b01000, 0b11110, 0b01000, 0b01000, 0b01001, 0b00110},  // t
    {0b00000, 0b00000, 0b10001, 0b10001, 0b10001, 0b10011, 0b01111},  // u
...
bool glyphPixel(std::uint32_t codePoint, int gx, int gy) {
    if (gy < 0 || gy > 6) {
        return false;
    }
    if (codePoint < 32 || codePoint > 126) {
        // Placeholder box for non-ASCII (CJK/emoji) code points.
        return gx >= 0 && gx < 4 && gy >= 1 && gy < 6;
    }
    if (gx < 0 || gx > 4) {
        return false;
    }
    return (kFont5x7[codePoint - 32][gy] & (1U << (4 - gx))) != 0U;
}

// Decodes one UTF-8 code point starting at *i; advances past it.
std::uint32_t decodeCodePoint(const std::string& text, std::size_t& i) {
    ...
    for (std::size_t k = 0; k < extra && i + 1 < text.size(); ++k) {
        ++i;
        cp = (cp << 6U) | (static_cast<unsigned char>(text[i]) & 0x3FU);
    }
    ++i;
    return cp;
}
