...
- 标题栏单行 **48px**（替代原 56px 顶栏 + 40px 菜单栏两行）：品牌标 28×28 + 标题 14px/650 + 菜单栏（项高 32px）+ 弹性拖拽区（右侧状态胶囊）+ 窗口控制。
- 窗口控制按钮 **44×32、通高贴合右上角**（Windows 惯例），图标 14px 描边 1.6；hover 弱表面，close hover 实心警示红（#c42b1c）。
