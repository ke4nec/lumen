...
- SDL 宿主在初始化与系统主题变化时查询 `highContrast`/`reduceAnimation`/`fontScale`：Linux 读取 portal D-Bus settings，macOS 读取 AppKit display preferences 与系统字体尺寸，Windows 读取 SystemParametersInfo 与 Accessibility 注册表；查询不可用时保留安全默认。
