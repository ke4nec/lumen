...
> 文档状态：实施设计；2026-09-22 状态：Windows UIA、Linux AT-SPI2、macOS NSAccessibility provider 已编入；Orca、VoiceOver、讲述人/NVDA 真实屏幕阅读器回环仍需在对应桌面会话人工验收。
...
M5 已完成语义契约收口（identity diff、invalid/hidden flags、语义 action、Recording bridge 回归证据）；M13 已增加可选 Windows UIA、Linux AT-SPI2 和 macOS NSAccessibility provider，真实屏幕阅读器（讲述人/NVDA、Orca、VoiceOver）人工验收仍需在目标桌面会话完成。此后控件面持续扩大（List/Tree/TreeList、Menu/ContextMenu/MenuBar、Splitter、自定义标题栏），当前 `SemanticsRole` 有 23 个枚举值并覆盖原始控件与后续控件；具体映射以源码及本文 §5/§6 为准。
...
4. AT-SPI2/NSAccessibility 按本文映射表实施，CI 负责编译/headless 回归，真实屏幕阅读器回环由平台验收记录负责。
...
createPlatformAccessibilityBridge（按编译定义分发）
  ...
  ├─ LUMEN_ACCESSIBILITY_PROVIDER_ATSPI（Linux）
  └─ LUMEN_ACCESSIBILITY_PROVIDER_NSACCESSIBILITY（macOS）
```
...
- **总线接入**：`$AT_SPI_BUS_ADDRESS`/`$XDG_RUNTIME_DIR/at-spi/bus` 取私有总线地址（回退会话总线 `org.a11y.Bus.GetAddress`）；不可达 → provider 保留但 `available()` 为 false 并给出诊断（无桌面会话属正常）。
- **注册**：向 `org.a11y.atspi.Registry` 的 `org.a11y.atspi.Socket.Embed` 注册根对象 `/org/a11y/atspi/accessible/root`，响应注册表写入的 `org.a11y.atspi.Application.Id`。
- **对象树**：每语义节点一个 D-Bus 对象路径 `/org/a11y/atspi/accessible/<hash>`（identity 哈希；根路径固定，跨更新稳定）。
- **接口映射**：
...
- **接入**：`nativeWindowHandle` 返回 NSWindow*（SDL 属性）；provider 侧 ObjC++ 取 `contentView`，经现代 `accessibilityChildren` 属性附加自定义 `NSAccessibilityElement` 根（旧 `accessibilitySetOverrideValue:forAttribute:` children 覆盖在当前 macOS 的 NSView 回读中已不被 honored，见 provider 注释）。
...
- **headless（ctest 常规）**：`tests/a11y_provider_tests.cpp`——工厂分流（未编入安全降级/编入名称一致）、Fake host 能力如实翻转、AT-SPI 对象路径跨更新稳定；Windows 继续覆盖 fragment 导航、属性映射（role/flags/bounds×倍率）、Invoke/Toggle/Value/RangeValue 回灌与事件。
...
| P2 Linux AT-SPI | org.a11y.Bus/Registry 直连 + §6 接口族 + 事件 | Orca 回环；CI 首跑编译/冒烟 | 已实施（回环待验） |
| P3 macOS NSAccessibility | §7 NSAccessibilityElement 树 + 通知 | VoiceOver 回环；CI 首跑 | 已实施（回环待验） |
...
- **AT-SPI/NSAccessibility 真实验收**：provider 已实现；没有桌面总线或 NSWindow 时能力如实为 false。Orca/VoiceOver/讲述人/NVDA 的人工回环仍是平台验收项，不由 headless CI 冒充完成。
