...
> 状态：待实施；本次只建立计划，不代表渲染器已经迁移或性能收益已经验证。
...
这些是待验证目标，不承诺整应用提升某个百分比。布局、文本排版、GPU 提交等待可能仍是主要成本。正确的直通与预乘实现应产生基本相同的画面；8 位量化和舍入差异需单独审查。
...
以下是编写时的源码事实。后续实施以符号和实时源码为准，旧注释、测试数量和历史性能报告不能代替复核。
...
拟在 `lumen::render` 引入 `enum class AlphaMode : std::uint8_t`，并在 `PixelBuffer` 现有字段尾部追加 `alphaMode`：
...
**命令记录**：写入新版本，按当前 v6 预计为 v7；实施时确认版本未被其他任务占用。新增 alpha 模式字段必须有固定宽度和固定枚举值。新读取器支持本次迁移前的 v6，把其图片负载解释为 `Straight`；不扩展到更早版本。未知版本、非法模式、截断和非法像素负载均拒绝，失败不修改调用方原有输出命令列表。旧读取器拒绝新版本属于显式版本边界，须记录在迁移说明中。
...
- [ ] P0.1 记录实际提交、工作树差异、工具链、依赖和构建选项；隔离并保留其他控件任务的未提交工作。
- [ ] P0.2 按第 2 节复查全部像素生产者/消费者；确认当前 source-over、coverage、Skia 读回与窗口路径。
- [ ] P0.3 建立透明底单色边缘、半透明叠层、图片再上传和 Skia 透明呈现的最小复现；建立独立目标算术参考模型以冻结量化误差规则，记录当前通过项和既有缺陷。
- [ ] P0.4 先补可在迁移前后复用的测量场景和转换计数，再采集第 6 节基线；测量工具改动独立于算法迁移。
- [ ] P0.5 保存现有固定场景和 Gallery 样本的原始帧、模式说明、黑/白底合成图及哈希。

**出口**：有可重复的 before 报告、窗口转换成本、复现记录和运行环境；未把历史 CPU/Skia 差异混作本次新增回归。若总帧成本主要在其他环节，记录优先级判断，不编造速度收益。

### P1：建立格式边界，先使所有消费者理解模式

- [ ] P1.1 增加 `AlphaMode`、字段、校验与幂等转换；显式 `Opaque` 证明规则写进公共接口注释。
- [ ] P1.2 所有图片接纳点、资源复制、命令上传保留模式；CPU 在本阶段仍可把输入转换为当前直通内部格式，迁移分支仅保留到 P2。
- [ ] P1.3 Skia 上传按模式选择 alpha type，读回声明实际格式；提前修正 SDL 格式分派，避免只标记输出却仍二次预乘。
- [ ] P1.4 完成窗口第 3.5 节矩阵、默认直通导出适配、新命令版本与 v6 读取；更新调用方和头文件契约。
- [ ] P1.5 增加模式与转换、序列化兼容、窗口路径选择、图片往返测试；CPU 仍保存直通的事实准确记录。

**出口**：生产者与消费者可同时处理两种有 alpha 的表示，默认手工图片兼容；CPU 旧算法仍正确，Skia 读回不会因新标记被重复预乘。此阶段不得提前宣称 CPU 渲染已迁移。
...
- [ ] P4.1 执行第 5 节矩阵，为像素差异生成模式、alpha、黑底/白底合成结果和空间分布报告。
- [ ] P4.2 为每个需要更新的哈希写明原因与对应样本；无关几何/颜色/字体变化逐项排除。
- [ ] P4.3 同机交错运行 before/after，按第 6 节比较提交、呈现准备和端到端耗时、内存及分配。
- [ ] P4.4 检查不透明场景没有引入隐式反预乘或每帧模式扫描；图片转换没有移到 draw 热路径。
- [ ] P4.5 完成一次覆盖源数据→缓存→回放→呈现→导出的全链路 review，修复后只重跑受影响检查及要求的全量门槛。

**出口**：正确性全部通过；性能报告区分已测收益、无显著变化和未测项目，满足第 6.3 节。哈希更新有依据，不能以新基准掩盖回归。

### P5：文档、兼容说明与交付

- [ ] P5.1 同步 renderer/CPU/Skia/资源/平台接口注释，更新标题栏与视觉系统中的旧直通呈现说明。
- [ ] P5.2 更新构建命令、截图格式说明、基准元数据和支持矩阵中的相关契约；历史记录采用追加解释，保留旧基线。
- [ ] P5.3 按第 9 节完成交付记录，记录重编译要求、`pixels()` 语义变化、命令版本及 v6 兼容规则。
- [ ] P5.4 全部出口满足后，向自用路线图追加完成记录；未验证的平台保留待办。最终代码与文档统一 review。
...
| 帧生命周期 | 即时 vs submit，全量 vs Preserve，多轮交错 damage，resize/DPI、双缓冲 | 同一 CPU 实现及模式下逐字节一致；`pixels()` 始终是已完成帧 |
...
| 场景 | 工具与状态 | 重点指标 |
| --- | --- | --- |
| card-grid、text-heavy、resource-upload | 现有 `lumen-scene-bench`；保持旧场景内容和参数 | paint/构建/layout 的 p50/p95、分配、命令数与资源上传成本 |
| 透明半透明叠层、裁剪边缘、图片叠加 | P0 新增固定 headless 场景或独立基准；当前 CLI 尚无这些场景名 | 单独渲染提交耗时、处理像素量、输出模式 |
| 图片一次上传后重复绘制 vs 每帧上传 | 现有资源场景加新定向场景 | 转换只发生在接纳边界；冷启动与稳态成本分别报告 |
| 1080p/4K 透明窗口 | P0 新增真实窗口测量或独立测量驱动；现有 headless bench 不覆盖 | 呈现准备、SDL 提交、整帧 wall time、scratch 容量与复制字节 |
...
命令从仓库根目录运行，实际依赖要求以 [统一命令表](build-commands.md) 为准。下面使用独立目录，避免覆盖现有构建；它们是后续执行命令，不是本次已运行记录。
...
按第 6.2 节分别保存每次输出。新透明/窗口场景的名称、可执行目标和参数在 P0 实现后补入本节；当前不得把它们当作已经存在的 CLI 选项。
...
- [ ] 公共 `Color` 仍为直通；CPU 内部帧/缓存统一预乘；Opaque 声明可信。
- [ ] 正常透明呈现无额外 alpha 转换；不透明正常路径无新增整帧反预乘。
- [ ] 图片/命令/窗口/导出全链路无格式丢失，v6 兼容和未知版本拒绝有测试。
- [ ] 全量与局部重绘、CPU/Skia/GPU、回退与真实窗口证据完整，未跑项明确保留。
- [ ] 视觉差异经审查，哈希更新有逐项依据，旧性能基线保留。
- [ ] 性能收益有实测，未显著变化与不适用项如实标注。
