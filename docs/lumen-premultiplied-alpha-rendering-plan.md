...
从 `7d226b4` 继续整体 review，修复基准汇总未绑定 manifest 来源、后续组新增阶段被遗漏的
问题；CPU/Skia 共 180 份原报告重新校验通过，所有汇总数值保持不变。另在 WSL Linux
补验时修复 macOS `.mm` 被非 Apple 桌面编译的既有构建阻塞及非 Windows 通知能力的旧测试
假设。构建、完整 CTest、字体跳过及平台边界见
[续验记录](perf-baselines/premultiplied-alpha-2026-09-20/follow-up.md)。P0–P5 记录保留当时事实，
本次没有重采样性能或修改渲染算法。
